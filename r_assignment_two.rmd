...
To calculate the 95% confidence interval using a normal approximation, I first calculated the mean of the differences between the model-standardized point estimate of the cumulative incidence ratio and bootstrapped point estimates of the cumulative incidence ratio, to which I will refer as $\overline{\Delta_\hat{\theta_i}}$:
...
SE_{\ln(\hat{\theta}_i)} \approx \sqrt{\frac{1}{n-1} \sum\limits_{i = 1}^{n}\left(\left(\ln(\hat{\theta}_i )-\ln(\hat{\theta})\right) -

\overline{\Delta_{ln(\hat{\theta_i})}}\right)^2}

$$

(To be honest, I did all of the above steps using the simple R code `sd.log.rr.boot <- sd(log(rr.samples) - log(stdized.rr))`.)
