...
The density plots for the sampling distributions for the RD and the RR drawn from bootstrapped samples of the data look normal and centered close to the point estimates from question one, as does the density plot for the sampling distribution of the natural logarithm of the RR.  Thus we can likely use normal approximations to calculate 95% CIs for both the RD and the logarithm of the RR. (I don't quite understand why the original assignment had us look at a density plot for the sampling distribution of the RR if we're calculating 95% CIs on the scale of the predictors, which corresponds to the natural logarithm of the RR.)
...
I calculated the Wald 95% CI for the RD using the estimated standard error above:
...
For Wald CIs to be valid, the following assumptions must hold:
...
5. The RD cannot be too close to zero or one, particularly in small samples, lest the CIs inaccurately include invalid RDs below zero or above one.[^2]
...
### Comparison to the bias-corrected and accelerated CI

The 95% CI for the RD using a normal approximation is (`r round(rd.ci.norm.apx[1], 3)`, `r round(rd.ci.norm.apx[2], 3)`), compared to the bias-corrected and accelerated CI of (`r round(ci.rd.bca$bca[4], 3)`, `r round(ci.rd.bca$bca[5], 3)`).
...
I first calculated the mean of the differences between the natural logarithm of the original model-standardized point estimate of the RR $\ln(\hat{\theta}) = \ln(`1.388`) = 0.328$ and the natural logarithms of the bootstrapped point estimates of the model-standardized RR, $\overline{\Delta_{ln(\hat{\theta_i})}}$:
...
5. The RR cannot be too close to zero, particularly in small samples, lest the CIs inaccurately include invalid negative RRs.[^2]

[^1]: Greenland S. Interval estimation by simulation as an alternative to and extension of confidence intervals. International Journal of Epidemiology 2004;33(6):1389–97. doi:10.1093/ije/dyh276. (Page 390.)

[^2]: Carpenter J, Bithell J. Bootstrap confidence intervals: when, which, what? A practical guide for medical statisticians. Statistics in Medicine 2000;19(9):1141–1164. doi:10.1002/(SICI)1097-0258(20000515)19:9<1141::AID-SIM479>3.0.CO;2-F. (Page 1150.)

### Comparison with bias-corrected and accelerated CIs

The 95% CI for the RR using a normal approximation is (`r round(rr.ci.norm.apx[1], 3)`, `r round(rr.ci.norm.apx[2], 3)`), whereas
the bias-corrected and accelerated CI of (`r round(ci.rr.bca$bca[4], 3)`, `r round(ci.rr.bca$bca[5], 3)`).
